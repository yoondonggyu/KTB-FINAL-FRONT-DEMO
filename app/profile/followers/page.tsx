...
const dummyFollowers: User[] = [
  ...
  { id: 2, nickname: 'estar', profileImage: null, interests: ['클라우드'], isFollowing: false },
  { id: 3, nickname: 'neon', profileImage: null, interests: ['AI', 'DevOps'], isFollowing: true },
  { id: 4, nickname: 'david', profileImage: null, interests: ['iOS'], isFollowing: false },
  { id: 5, nickname: 'alex', profileImage: null, interests: ['백엔드'], isFollowing: true },
  { id: 6, nickname: 'jane', profileImage: null, interests: ['프론트엔드'], isFollowing: false },
  { id: 7, nickname: 'mike', profileImage: null, interests: ['클라우드', 'DevOps'], isFollowing: true },
  { id: 8, nickname: 'sara', profileImage: null, interests: ['AI'], isFollowing: false },
]

export default function FollowersPage() {
  const router = useRouter()
  const [activeTab, setActiveTab] = useState<'followers' | 'following'>('followers')
  const [followers, setFollowers] = useState<User[]>(dummyFollowers)
  ...
  const confirmUnfollow = () => {
    if (selectedUser) {
      setFollowers(prev => prev.filter(u => u.id !== selectedUser.id))
  ...
  const handleFollow = (user: User) => {
    ...
    if (selectedUser && selectedUser.id === user.id) {
      setSelectedUser({ ...user, isFollowing: true })
    }
  }

  const handleFollowButtonClick = (e: React.MouseEvent, user: User) => {
    e.stopPropagation()
    if (user.isFollowing) {
      setSelectedUser(user)
      setIsUnfollowModalOpen(true)
          ...
          <div className={styles.profileModal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.profileModalContent}>
              ...
              <div className={styles.profileModalActions}>
                ...
                <button 
                  className={`${styles.profileModalFollowButton} ${selectedUser.isFollowing ? styles.followingButton : ''}`}
                  onClick={(e) => handleFollowButtonClick(e, selectedUser)}
                >
